...
class BitcoinMonitor:
    ...
    def render(self, md, funding, oi_stats, top, low, trend=None, mom=None):
        ...
        header = [
            f"  {self.title}",
            f"  監測時間  {now}     邏輯來源  {src}",
            # 「刷新週期 60 秒」與原本 quote 區的「數據時效 即時 60s｜…」講同一件事，
            # 2026-08-26 合併到這裡：**省一列**（儀表板原本剛好比終端機高一列，
            # 上下各被吃掉一行），且表頭本來就有大片閒置橫向空間（原寬 39，
            # 合併後最寬 84，仍在版寬下限 2*_MIN_COL_W+2=102 之內，不撐寬版面）。
            f"  交易對    {self.symbol}     刷新  即時 60s｜日線·地板·外部 {data_age}",
        ]
        quote = [
            f"  現價          ${md['close']:>12,.0f}",
        ...
        #    → 2026-08-26 起併入表頭第 3 行（見上方），本區不再另佔一列。
