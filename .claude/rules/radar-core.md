...
**`BTC_WATCH.py` 在本 repo 根目錄**：OI 用 `openInterestHist`（5m×13 滾動清洗＋1d×30 分位），
不用相鄰 60s 差值（已失效）；
...
馬丁重啟偵測器已整組移除（README v3.51）；本條只留它留下的兩條通則：

- **偵測器的觸發條件不可與「它要保護的那件事」同時成立**——只在跌破警報價才檢查
  防守階梯，等於要用階梯的那一刻才發現它早就壞了。
- **偵測器「取不到資料」時不可靜默**——那和「偵測到沒事」是兩件事。現行機制：
  `daily_line_notify._maybe_send_data_gap_alert`／`_clear_data_gap_flag`，套用在會影響下單的
  四個哨兵（升槓桿窗口／熊底 D3／D3 網格緩衝／套保建倉）；推一次、恢復清旗標，一天三場不洗版。
  逃頂雷達與合成行動刻意不納入（SOP 附錄 F-4：不作賣出依據）。
  紅線測試：`tests/test_alert_logic.py`、`tests/test_hedge_batch_alert.py`。
