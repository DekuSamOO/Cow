...
- **docs**: 依官方「CLAUDE.md 目標 200 行內、越長遵守度越低」與 Lost in the Middle／IFScale 的注意力證據，
  把「只在改特定檔時才用得到」的陷阱**逐字**搬進 7 個 path-scoped 規則檔：`streamlit-ui`（陷阱 1/2/3/5/7）、
  `service-data`（fallback chain＋4/6/16/17）、`radar-core`（三大核心實作細節、改 WEIGHTS 三件事、8/9/10/22）、
  `tw-us-watcher`（台股/美股守則＋18/19/20）、`news-gemini`（12/13/14）、`requirements`（15）、`testing`（23）。
- CLAUDE.md 只留跨功能守則（執行指令、每日自動 push 通道、對外推播閘門、單一真實來源、
  OI×Funding 折減與反指標移除、陷阱 11/21、受保護決策）＋**標題索引表**，外部引用照舊引標題即可找到。
- **chore(.gitignore)**: `.claude/` → `.claude/*` ＋ `!.claude/rules/`，否則搬出來的規則不進版控。
- **verify**: 獨立腳本比對舊版所有反引號／粗體／數字片段，377 個事實片段在新版全集 **0 缺漏**；
  7 個規則檔的 `paths` 逐條對得到實際檔案。

### v3.55 (2026-09-15)
**行情快取改絕對路徑**——同一支 `sop_status.py` 在不同目錄跑，D3 低點不一樣。

- **fix(market_data)**: `BTC_CSV` 原為相對路徑 `"db/cache/BTC_HISTORY.csv"`，快取跟著執行目錄走：
  在 vault 目錄跑會在 vault 另建一份快取；Cow 目錄那份舊快取的 **06-30 列是殘缺日線**
  （收 59,836.28、量 711 BTC；15m DB 真值 58,624.71、量約 1.96 萬），且卡在歷史中間、
  `REFETCH_TAIL_DAYS` 永遠重抓不到。結果本機在 Cow 目錄跑讀到本波低點 59,577.01 @ 06-28，
  在 vault 目錄跑讀到 58,624.71 @ 06-30；vault ROAD 閘門表因此記錯一週（距低 90 天日期差 2 天）。
  → 改為 `os.path.join(_REPO_ROOT, "db", "cache", "BTC_HISTORY.csv")`，比照 `local_db_reader.DB_DIR`。
- **ops**: 本機舊快取備份後刪除，由 `fetch_market_data()` 自 15m DB 重建（06-30 收 58,624.71）。
  雲端哨兵本來就沒有這份快取（gitignore、每次重建），**推播判定不受影響**
  （2026-09-15 哨兵 log「距低 77 天」＝ 06-30）。
- **verify**: 從 vault 目錄執行 `sop_status.py`，vault 不再產生 `db/`，低點 58,624.71 @ 06-30 與雲端一致；
  全套 **615 passed**。

### v3.54 (2026-09-11)
**殘缺的最後一根日線被當成收完的日線**——套保第 2 批該響沒響，而且全程無聲。
這是本日第三個、也是唯一一個**算錯數字**的缺陷（前兩個是狀態接力與殘留）。

事故鏈：collector 每天 09:00 本地跑、抓到當日 01:00 UTC 就 push，所以 15m DB 的
**最後一天永遠是只有 5 根 K 棒的殘根**。平常無害（那是「今天」，`closed_daily_rsi()`
本來就排除當日）。2026-09-11 那天 collector 被 Ctrl+C 中斷（`LastTaskResult`
`0xC000013A`）沒 push，雲端 checkout 到的 DB 最後一天變成 **09-10 的殘根**，
隔天它就成了「昨天」而逃過當日排除：

```
09-10 只有 00:00~01:00 的資料 → 收盤 78,180.69（真值 76,568.72，差 2.1%）
→ 哨兵算出 RSI 58.97（真值 52.83）→ 第 2 批（<55）不算到期 → 不推、不警示。
```

- **fix(local_db_reader)**: 新增 `drop_incomplete_last_day()`，`read_btc_daily()`
  重採樣後丟掉「資料沒收到當天 23:45」的最後一根。**只丟最後一根**：中間缺天是
  交易所停機之類的歷史事實，一起丟等於改寫回測輸入。
- **fix(sentinel_board)**: `crosscheck_daily_rsi()` 自己也重採樣一次，套同一道守門。
- **feat(notify)**: 新增收完日線的落後守門 `HEDGE_MAX_CLOSED_BAR_LAG_DAYS`=1。
  `closed_daily_rsi()` 排除當日，最新只可能是昨天；落後 2 天以上代表資料源少了一天，
  改推「哨兵失明」告警並**不推任何建倉指示**，不再拿舊收盤照算。
- **為什麼兩源對拍擋不到**：對拍守門只在「主源認為這批到期」時才啟動。主源偏高
  就整批不算到期，連「先不要建倉」都不會推。**主源自己錯的時候那道守門是空的**——
  而且在雲端兩源其實同源（chain 第一順位是 Local SQLite，讀的就是同一份 15m DB）。
- **test**: 新增 `tests/test_incomplete_last_day.py` 10 條（殘根要丟／收滿不丟／
  邊界在 23:45／中間缺天不連坐／空表安全／過期不得靜默／新鮮不得誤擋）。
  全套 615 passed；**負向驗證**：兩道守門各自停用，各 2 failed。
  以事故當時那份 DB 重播：修復前印 RSI 58.97 且靜默，修復後丟掉殘根 → 落後 2 天 →
  改推告警。
- **fix(test)**: `test_hedge_batch_alert.py` 與 `tests/core/test_hedge_crosscheck.py`
  原本寫死 `rsi_closed_date="2026-09-02"`，新守門一上線會整批誤判成過期。
  改成相對今天算，對拍日期改用 `x_lag_days` 表達落差。

### v3.53 (2026-09-11)
**斷鏈留下的兩筆殘留修乾淨**——v3.52 修掉了「狀態為什麼會回到十天前」，
但**那十天在 state 裡留下的東西還在**，而且兩處都在用「筆數」思考、看不出中間有洞。

- **fix(notify)**: `score_history` 的 Δ 基準加新鮮度守門。舊碼取「最近的前一筆」，
  斷鏈後那筆是 **08-31**，於是 09-10 的 Δ 實際是跟十天前比，畫面上照樣標成「vs 昨日」。
  → 超過 `SCORE_DELTA_MAX_GAP_DAYS`=3 天就不給 Δ（容得下週末或單場漏跑，容不下斷鏈）。
- **fix(notify)**: `score_history` 的保留策略由「排序後留 8 筆」改成
  **先按日期丟掉 `SCORE_HISTORY_KEEP_DAYS`=14 天以上的，再留 8 筆**。
  舊寫法在有洞時筆數看起來還是滿的——線上那份是 8 筆橫跨 17 天、中間缺 9 天。
- **fix(notify)**: 週報的「本週」改用日期界定（近 7 個日曆天），不再把
  `score_history` 全部筆數當成本週。舊寫法會把兩週前的分數算進「週高／週低」。
- **fix(notify)**: 一次性校正 `hedge_batch_1_date`。這個欄位記的是**推播日**
  （推播成功後才寫），第 1 批被重推兩次、每次覆寫，線上只剩最後一次重推的 09-10。
  真正的首推是 run `34188465889`（2026-09-08T04:53:33Z）。
  → `_migrate_hedge_batch_1_date()` 校正回 09-08 並留痕，辨識法窄到只咬那一筆。
  比照 `_migrate_legacy_closed_days()` 的作法。
- **fix(sop_status)**: 批次狀態的字面由「已建（…）」改成「已推播（…）」——
  哨兵不知道使用者實際幾點下單，舊字面害人把推播日讀成建倉日（兩批實際都是
  使用者跑在哨兵前面幾小時建的）。另外 `score_history` 改印筆數與日期跨度，
  有洞時直接標「⚠️ 有洞：缺 N 天」，不再只印最後一天。
- **test**: 新增 `tests/test_score_history_freshness.py` 10 條。全套 605 passed；
  **負向驗證**：四道守門逐一停用，各自 1~2 failed，沒有空轉的紅線。
- **fix(test)**: `test_alert_logic.py::test_attach_score_deltas` 的基準日原本寫死
  `2020-01-01`，名義上叫「前一日」實際是六年前，新守門一上線就整條不給 Δ。
  改成相對今天算。
- **不做的兩件**：①「首推日 write-once」查過是死碼——重推只發生在旗標消失時，
  而旗標與日期同存於一份 state、會一起消失，`setdefault` 在那情境永遠空跑；
  ② 09-01~09-09 那九天的分數不回填——分數吃當時的行情狀態，用今天的資料重算
  是 look-ahead，會製造一個無法驗證的數字，改成把洞標示出來。
...
- **remove(notify)**: 刪除 `maybe_send_mart_restart_alert()`、`detect_mart_restart()`、
  `_mart_restart_lines()` 與私有設定 `MART_TP_BASELINE`。LINE 哨兵由 7 個減為 6 個
  （`core/sentinel_board.py` 清單與顯示列同步重編號）。
  三個移除理由，缺一都不足以刪：
  ① **取不到資料**——GitHub Actions 的美國 IP 遇 Binance 451、Kraken 備援也空，
     `detect_mart_restart` 恆回 `None`，抽查 2026-08-23/08-24/09-03~09-06 六場全印
     「馬丁重啟偵測不可用（行情取數失敗），略過」；
  ② **從未推播過一則**——狀態 artifact 裡沒有 `last_mart_restart_key`，
     而兩台馬丁 2026-08-24／08-25 就已跨過推斷止盈價（基線 08-22，後高 $82,300）；
  ③ **追蹤對象已退場**——六台幣本位網格全關、無槓桿部位，`DEFENSE_LADDER` 語義
     已由「補保證金」降為「彈藥動員」，沒有階梯要防守。
  防守推播的「執行前必對帳重算」提醒**保留為固定一行**（它是靜態警語不是哨兵），
  由 `tests/test_defense_ladder.py::test_message_keeps_static_recalc_warning` 守住。
- **fix(ops)**: 同步指紋由五元組改回四元組（拿掉 `mart_tp_baseline`）。
  本地與 Actions 走同一份程式碼，兩邊同時改變、指紋照樣一致；
  Secret `DEFENSE_CONFIG_JSON` 裡殘留的 `mart_tp_baseline` 鍵會被忽略，**不必動 secret**。
- **fix(leverage)**: `advance_batches` 關窗那條補同日去重（`lev_last_closed_date`）。
  修正前 `lev_closed_days` **數的是哨兵場次不是日曆天**——工作流一天跑三場、每場 +1，
  `WINDOW_RESET_DAYS = 90` 實際 30 個日曆天就到，會把「窗口暫時關掉、之後續接」
  誤判成「換一個熊市階段」而重頭投第 1 批（同一個熊市可能投超過 6 批，
  違反 vault「1b 開窗後分批執行研究」七方案對決選出的「最多 6 批」）。
  實證：狀態 artifact 2026-08-25 為 2、2026-09-06 為 38，相隔 12 天恰好 +36＝12×3。
  開窗那條本來就有去重（`lev_last_open_date`），本次補上對稱的那一半。
...
v2 自 2026-07-06 實作完成後一直沒啟用——回放三準則只過一條，兩個未解問題卡在那裡。
本版把兩個問題都修掉、把寫錯的那條準則改對，然後才切。

- **feat(season)**: 市場軸 **3 級 → 4 級**（新增 `deep_bear`：`dd < -0.30 且 not up`），
  象限表 **12 → 16 格**。原因：v2 最深只到 `bear`，**-30% 與 -83% 落在同一格**——
  2018-12-15 的 `dd=-83.2%` 歷史級崩盤，v2 仍只給 `autumn`。
  這造成 v1/v2 回放 1,439 天差異裡有 **1,019 天（70.8%）純粹來自這個缺級**。
  **既有門檻（-10%/-20%）與防抖天數（3 日）一個字都沒動**，只新增一級。
- **fix(season)**: 防抖加**逃生門**——站上年線即刻脫離空頭 family，不等 3 日；**只開單向**
  （進入仍需連續 3 日）。理由是語意自洽：`bear` 的定義本身含 `not up`，
  讓 `up200=True` 的日子維持 `bear` 是內部矛盾。實測 2021-08/10/12 與 2025-03 共 12 天
  因防抖卡住而**把牛市回檔判成熊底**（v1 `bull_peak` vs v2 `bear_bottom`），修後**歸零**。
- **fix(notify)**: `builders._build_forecast_box` 原為**二元判斷**（非 `bear_bottom` 即當牛市），
  `observe` 會渲染成「🚀 牛市最高價預測」，配上 `daily_line_notify` 已有的 `None→0` 轉換，
  實際會推出「**🚀 牛市最高價預測 $0 / $0 / $0**」——不崩潰，但方向完全相反。新增 observe 分支。
- **feat(notify)**: `_build_bottom_eval_box` 在 `observe` 且依據含「四季論」時加註記。
  因為 `bottom_floors` 直接呼叫 `project_bear_bottom()`、**不受 `SEASON_ENGINE` 影響**，
  切 v2 後同一張卡會同時出現「不出目標價」與 `final_low $33,035`。
  處置採「保留數字＋標明僅供參考」，不砍數字（砍了 `final_low` 會退化成只剩電費地板）。
- **test**: 新增 `tests/core/test_forecast_box_observe.py`（8 項，含 P-1/P-2 守門，
  並做過**負向驗證**：停用修法後 3/5 失敗）、`tests/season_v2_criterion2.py`（準則 2a/2b 實證）、
  `tests/season_v2_inspect.py`（人工抽查工具）。`test_season_v2.py` 24 → 35 項。
  **全套 547 passed, 0 failed**。
- **docs**: 回放三準則現況——準則 1 由不符轉**符合**（observe 型目標格 15.5% → **63.5%**）、
  準則 3 通過（切換次數 v1 79 / v2 37）；**準則 2 改寫為 2a/2b**（下跌段須一致／反彈段允許轉
  `observe`），原措辭與 v2 設計目標從第一天就矛盾。兩輪熊底**下跌段皆 0 分歧**。
- ⚠️ **回滾**：改回 `"v1"` 即可，v1 路徑零改動、已實測。
...
不會知道那個建議背後的複合分數其實 train/holdout 符號會反轉、或只有否決端過了獨立驗收。
本版把這件事**從註解搬到畫面**，不動任何決策矩陣或門檻。

- **feat(action)**: `core/action_ensemble.compute_composite_action` 新增選填參數 `notes`，
  回傳新增 `confidence_note` 欄位——依觸發分支（ESCAPE_DRIVEN／VALUE_DRIVEN／純趨勢）附對應
  文字。crypto 呼叫端統一吃新常數 `CRYPTO_ACTION_NOTES`（集中管理，避免各呼叫端各寫一份漂移）；
  不傳 `notes` 行為與舊版完全相同（向後相容，`tests/position_calib.py`／`scripts/tw_position_calib.py`
  等既有呼叫端不受影響）。
- **feat(action)**: `BTC_WATCH.py`（終端機）／`handler/tab_macro_compass.py`（Streamlit）／
  `scripts/daily_line_notify.py` + `service/notification/builders.py`（LINE：每日卡片、操作訊號
  翻轉警報、週報）四處全部接上 `confidence_note`，確保三邊看到的信心註記一致。
  `_build_advice_box` 新增選填 `note` 參數（向後相容，另外兩個既有呼叫端不受影響）。
- **fix(watcher)**: `watcher.py` 進場畫面（輸入代號那一步）移除哨兵總覽——七個 LINE 哨兵
  全是 BTC 專屬，塞進 2330 或 NVDA 的選代號畫面只是雜訊。`_sentinel_board()` 無其他呼叫端，
  一併刪除，不留死碼。BTC 儀表板內部既有的哨兵摘要（`BTC_WATCH._sentinel_board_rows`）不受影響。
- **test**: `tests/core/test_action_ensemble.py` 新增 8 個測試（`notes` 預設 `None` 的向後相容、
  三種觸發分支各自對到正確註記文字）。**513 passed**（原 505）。
...
響了也說錯話」的缺陷，並新增熊底確認 D3 哨兵與 BTC_WATCH 終端機兩行摘要。
**真實部位數字一律只在 `config_private.py`／`DEFENSE_CONFIG_JSON` secret／vault，本檔與所有公開檔不列。**
- **fix(notify)**: **分批計數會被小反彈整組重置**（本次最嚴重）。回測 `D1_batching.py` 的
  gap 用的是**訊號日**索引——閘門暫時不成立的日子只是不計數、不重置建構；而 v3.44 實作用
  「日曆日 ＋ 開窗即 `lev_batches_sent=1`」，兩者不一致。實際後果：2018 那段窗口被 2~6 天的
  小反彈切成 7 截＝重置 6 次，**六批永遠投不完**。改為累計 `lev_signal_days`，關窗只暫停
  不歸零，連續關窗逾 `WINDOW_RESET_DAYS`=90 天才視為換一個熊市階段而重置。
- **fix(notify)**: **推播文案已過時**。舊文案寫「開新 2x 前確認：預估強平價 ≤ 當時價 × 0.5」
  與一整段 No.6 的去留指示——2026-08-24 已拍板改 2X 無額外保證金（強平價＝開單價 × 0.667）、
  且 No.6 已平倉。文案照舊等於在窗口真的開啟時給出錯誤的下單參數。已全數改寫並加上
  「開單後務必讀 App 實際強平價比對，差 > 1% 立即停止」。
- **fix(notify)**: `event is None` 同時涵蓋「窗口沒開」與「窗口開啟中但未到下一批」，
  會在窗口開啟時誤印「窗口未開」。兩者語意不同、不可共用文案，已拆成獨立分支；
  窗口未開時順帶用 `trigger_price` 印出「還差多少 % 到門檻價」。
- **feat(core)**: 新增 `core/leverage_window.py`——閘門、觸發價、分批推進、熊底低點、D3 判定、
  兩行摘要全部收在此處，純計算零 IO，供 LINE 哨兵與 BTC_WATCH 共用（詳見「架構」段）。
- **feat(notify)**: 新增 `maybe_send_bear_bottom_confirm_alert()`（熊底確認 D3）並掛進
  `__main__`。判準取 `_governance/PREREG-season-falsification.md` 的**預簽**定義：自最低點
  反彈 ≥50% 且距最低點 ≥90 天。回測三次熊底（限「自 ATH 已跌逾 30%」後才評判）中位延遲
  +99 天、**提早喊底 0/2**；而「站回 200 週均線」「自低點彈 30%」等快訊號提早 51~304 天、
  觸發價比真底高 40~197%（2026-05-10 就會喊底，之後還跌了三成）——**故不自創更快的條件**。
  觸發語意＝本輪熊市視為結束 → 升槓桿窗口可能不再開 → 馬丁的 USDT 該考慮換現貨。
- **feat(watch)**: `BTC_WATCH.py` 新增哨兵兩行橫向摘要，塞進「即時行情」區右側的閒置空間
  （不另開面板、不增加垂直高度）。判定隨日線每小時算一次而非每 60s 重算——那串
  argmax／布林遮罩／argmin 都是 O(n) 全表掃描（df 常態約 1500 列），放進 render 等於
  每小時白算 59 次。
- **fix(test)**: `tests/test_defense_ladder.py` 的 skip 邏輯把「`config_private.py` 不存在」
  與「存在但 `LIQ_BASE is None`＝無槓桿部位」混為一談，訊息寫「檔案缺失」但其實檔案在。
  拆成三態，且無部位時改驗**無部位的不變量**（`POSITION_USD` 須同為 None、階梯 `liq_after`
  須全為 0.0）——若有非 0 值，代表設定正在描述一個不存在的部位，必須擋下。
  同時修 `__main__` 直呼舊測試名（該函式已於 2026-08-21 改名，此處未同步）。
- **refactor(simplify)**: 收尾 `/simplify` 四角度平行審查後套用，**零行為變更**（已用舊
  pandas 寫法獨立重算對拍 ATH 天數／低點值／低點日期／距低天數四項，全部一致）：
  ①「自 ATH 跌逾 30% 後的最低收盤」原本在 `BTC_WATCH` 與 `daily_line_notify` 各手刻一份、
  30% 門檻硬編兩處，抽成 `find_bear_low()` 收進單一來源；②`d3_status` 的首參原名
  `close_series` 卻只讀 `[-1]`，害呼叫端得包 `[price]` 這種殼子，改為誠實的 `current_price`；
  ③`maybe_send_leverage_window_alert` 隔空 60 行的 `ok` 旗標改為就地 `if not _push(...): return`；
  ④`BTC_WATCH._sentinel_rows` 整段吞光一切的 `except Exception` 會讓欄位改名／常數搬家這類
  程式錯誤靜默消失，改為在每小時的日線刷新路徑印出原因。
- **test**: 新增 `tests/test_leverage_window.py`（11 項，零網路、數字全用顯假值），
  全套 **459 passed, 2 deselected**。`scripts/defense_config_check.py` schema OK、
  指紋與 Actions Secret 對拍一致（指紋不落公開檔，同 v3.43 既有政策）。
...
`stock_profile.py` 連改三輪留下的殘留為主，**未動任何配重、門檻、分位口徑**。
- **refactor(service)**: 新增公開 `fetch_quote_meta`，`scripts/stock_profile.py` 的 `_yahoo_meta`
  刪除改吃它。原本該檔跨模組 import 三個私名（`_session`／`_tw_candidates`／`_YF_CHART`）
  並複製一份候選迴圈——與稽核 W-7（watcher 曾直接 import `_vol_pctile`）同一個錯，
  service 端調 UA／timeout／`_RESOLVED` 語意時第三份不會跟上。
- **refactor(core)**: 分位公式收斂成 `relative_universal.midrank_pctile` 單一出口
  （`vol_pctile` 的股數分位與 `stock_profile` 的成交額分位原本各寫一份**逐字相同**的
  midrank，而兩個數字就並列在同一份報表上給使用者比較）。
- **refactor(core)**: 新增 `_vol_population`，`vol_pctile`／`vol_snapshot` 共用同一份母體。
  修掉兩件事：①`vol_snapshot` 原本先自己 `_vol_series` 一次、再呼叫 `vol_pctile` 又清洗一次；
  ②`n_pop` 原本用 `len(v) - window + 1` **第二條公式**推導母體筆數 —— 今天數值恰好相同，
  但 `vol_pctile` 的母體規則一改，watcher 畫面上「母體＝… N 筆」就會**靜默說謊**，
  而那行字的存在目的正是讓使用者相信母體。移除無任何消費端的回傳鍵 `ref_ma`（＝`ma/ratio`）。
- **fix(test)**: `test_momentum_json_omits_backtest_rejected_stance` 原本 monkeypatch 掉
  `time_series_momentum` 後**自己組一個 dict 再對它斷言**，完全沒執行到 `stock_profile`
  的任何一行——`momentum` 真把 stance 加回去它照樣綠燈，**是假證據**。
  抽出 `_momentum_block()` 讓這個「不准送出去」的不變量真的可測，並先斷言引擎本身確實
  產出 stance／label（否則「輸出沒有 stance」可能只是因為引擎根本沒產生它）。
- **clean(scripts)**: 死參數 `_climber_history(days=)` → 常數 `_CLIMBER_DAYS`（它是分位定義
  的一部分，掛成參數等於邀請呼叫端去傳）；SELECT 移除全檔無消費端的 `Dealer_BuySell`／
  `Short_Balance`（留著會讓人以為自營商與券資比有進評分）；魔數 `486` → `_BARS_2Y`；
  死變數 `sig`；`_coverage` 不可達的 `else None`；`_yahoo_meta` docstring 宣稱含 52 週高低
  （反向指引——52 週刻意改用日曆 365 天，見 `profile()` 註解）。
- **clean(term-ui/watcher)**: `_pack_segs` 空輸入守衛（該函式本來就回 `[]`）與 docstring 矛盾；
  框線補白的 `max(0, …)` 恆不觸發且在 `n_rows` 次迴圈裡重算同一值 → 提到迴圈外；
  `render()` 內 `snap` 曾在同一函式綁定兩次指兩件無關的事（量能快照／E3 訊號快照）→
  前者改名 `vol_snap`；`load_plans_cached()` 同一輪呼叫兩次各丟一半回傳值 → 合併成一次。
- **verify**: `pytest tests/ -q` → **420 passed, 2 deselected**（與整理前逐項相同）。
  另做**獨立對拍**（不使用被改的那套邏輯）：把整理前的 `vol_pctile`／`vol_snapshot` 原碼
  逐字抄成 `_old_*`，在 9 種長度 × 3 seed × 5 種資料形態（含 NaN／零量／定值序列／無 volume 欄）
  × window(1/3/5/20) × drop_last × ref_window 共 **4,384 組**上比對 → **位元級相同、0 筆不一致**。
  端到端實跑 `stock_profile.py 2330 --json` 通過（全史分位 0.9352／近 2 年 0.6821／
  逃頂 43／抄底 53，與整理前記載一致）。
- **審查發現但刻意未動（會改行為，需使用者裁定）**：
  1. **ATR 有兩份實作**。`stock_profile.short_term_traits` 自算 `tr.rolling(14).mean()`（SMA），
     但 `profile()` 上一行才跑完 `calculate_technical_indicators`，`core/indicators.py` 的
     `ATR` 是 pandas-ta 的 **Wilder RMA**；`core/risk.py` 檔頭正是為避免這種漂移而存在。
     → 同一支股票在 watcher 與 stock_profile 會印出**兩個不同的「ATR(14) x.xx%/日」**。
  2. **`fin_chg_pct` 口徑不一致**。repo 每一處都是**日**變化（`service/tw_chip.py:144`、
     `scripts/tw_calib_extract.py:54`〔校準正本〕、`tw_position_calib.py:80`、
     `tw_variant_backtest.py:76`「現役」，且 `relative_low_tw.py:52`／`relative_high_tw.py:198`
     的 note 明寫「融資餘額日變化」），但 `stock_profile._margin_chg` 是**近 5 日**變化
     （`m.iloc[-1]/m.iloc[-6]-1`），仍以同一個 key 餵給 `compute_relative_low_tw`——而抄底
     leverage 是滿分 40 的最強維、門檻 −1/−3/−5% 是在日變化上校準的。5 日累積變化分布較寬，
     等於系統性把該維推向高分，可能正是 `_low_position_note` 要處理的「leverage 單維獨大」成因。
  3. 其餘僅記錄：`price_source` 判斷仍落在消費端（service 缺 `resolve_live_price`，
     watcher 也各判一次）、chip bundle 的 key 與 `tw_chip.get_chip_bundle` 契約已漂
...
- **fix（我上一輪修錯方向）**: 活躍度主指標由「成交額分位」改為 **`turnover_ratio_5_60`
  （成交額量比）**。v3.38 把股數分位換成金額分位，理由是股數隨股價漂移——但**名目金額
  同樣漂移，只是方向相反**：2330 分年日成交額中位數 2016 38.3 億 → 2026 834.5 億
  （**21.8x**），expanding 全史分位讓近年天數天然落在高分位。實測 **2330 全史 94 vs
  近 2 年滾動 68，差 25 個百分點**（1101 對照組亦差 19），那一截是股價成長不是活躍度。
  → 主指標改用天生平穩的量比（自己比自己，分子分母同單位同期）與週轉率（無量綱）；
  新增 `turnover_pctile_2y`（滾動 2 年）；全史分位降級為「名目金額的歷史位置」參考欄。
  股數分位維持不動（Cow 台股量能維校準口徑 AUC 0.648，不可換單位）。
  agent 定義加**仲裁規則**：量比與分位矛盾時一律以量比為準（2330 量比 0.64x 縮量
  vs 全史分位 94 就是這個情況）。
- **fix**: 新增 `price_source`，比照 Cow service 層既有的來源追蹤慣例。驗收時 2330 的
  `price` 與 `prev_close` 恰好同值、漲跌 0.00%，**從輸出無從分辨是即時報價還是回退到收盤**。
- **fix**: 52 週高低由 `yq.tail(252)` 改為 **日曆 365 天**，並新增 `hi_lo_52w_basis` 標明
  「Yahoo 未還原、與即時現價同基準」。252「根」對台股是 54 週；不標基準則會被誤認為與
  技術面（climber 還原序列）口徑不一致。
- **fix**: 抄底的位置交叉檢查**下沉到引擎**（`radar.low.position_note`）——原本這道防線
  只寫在 agent prompt 裡，只存在於 prompt 的門檻遲早會被漏掉。leverage 維佔總分過半且
  52 週位置 ≥60% 時自動出警語。涵蓋率警語同步掛到 `radar.coverage_note`（原本只掛在
  活躍度那兩行，只看雷達區塊的讀者不會知道母體有缺漏）。
- **fix**: 移除語意曖昧的 `as_of`（與 `chip.as_of` 同名不同義，留著遲早再騙人一次），
  改用 `run_at`／`data_as_of`；`chip.institutional` 補 `unit`（climber 存的是**股數**，
  不標的話消費端無從分辨張/股/仟股）。
- **test**: 新增 3 項（量比平穩而全史分位飽和／位置交叉檢查只在單維獨大＋高位置才觸發／
  來源追蹤有印出）。`pytest tests/ -q` → **417 passed, 2 deselected**。

### v3.39 (2026-08-12)
`stock-evaluator` agent 首次驗收，修掉它抓到的六個缺陷。
兩支 agent（2330 新建卡／6782 複評）在乾淨環境跑，共回報 12 項、去重後 6 項成立且全修。
- **fix（阻斷級）**: `main()` 加 `sys.stdout/stderr.reconfigure(encoding="utf-8")`。
  Windows 中文主控台預設 cp950，印不出 🟢🟡🔴 → `UnicodeEncodeError`、exit 1、**零輸出**，
  `render()` 與 `--json` 兩種模式都炸。**開發期每次測試都習慣性前綴 `PYTHONIOENCODING=utf-8`，
  等於每次都繞過這個 bug**——要有人在乾淨環境跑才會現形。
- **fix（同 v3.36 的病再犯一次）**: 分位母體標示改用 `tech_from`/`tech_bars`，不可用
  `history_from`。分位真正算在 `tech_df` 上（台股＝climber、美股＝Yahoo），但 render 拿
  Yahoo 的起日去標 → 台股顯示「2016 起全史」，實際只有 400 根 **≈1.7 年**。
  同時把 `_climber_history` 的 `days` 400 → **3000**（涵蓋 climber 2016 全史，與台股量能維
  的校準口徑對齊）。**修完數字實質改變**：2330 活躍度 62 → 94 分位；6782 量能維 18/18
  「爆量見頂」→ 6/18「偏高」，逃頂總分連帶下修。
- **fix（新增 `tech_coverage`）**: 「X 起 N 根」字面為真卻**暗示連續性**——climber 的 6782
  首列 2021-01-13、893 根看似 5.6 年，實際 2021 年只有 1 列、2022 年 25 列，2023 才完整。
  涵蓋率 <90% 時畫面明示「涵蓋率僅 N%，母體沒有起始日看起來那麼長」。
- **fix（用固定文字冒充判讀）**: 移除 `render()` 裡一句**無條件**樣板
  「ATR 明顯較大＝波動主要發生在開盤那一跳」——它不看任何數字，對跳空僅 5% 的 6782 也照印。
  且兩數口徑不可相減（ATR 是 14 日**平均**、振幅是 60 日**中位數**，右偏分布下均值本就高於
  中位）。改為指向 `gap_over_2pct_ratio` 這個直接量測。agent 定義裡複製的同一條錯誤推論一併刪。
- **fix（假時間戳）**: `as_of` 是執行時鐘不是資料時間 → 新增 `run_at`／`data_as_of`，
  報表頭列同時標技術面、籌碼、TDCC 三個截止日並註明現價為即時（盤中同日 09:25 與 09:29
  實測現價 2,400 → 2,405，「同一天跑兩次同一組數字」只對收盤來源的欄位成立）。
- **fix（台股中文名）**: Yahoo 對台股回英文登記名（6782 → "Visco Vision Inc."），
  改為 climber `stock_list` 的中文全名優先、英文存 `name_en`。原本消費端要寫中文標題
  只能憑記憶翻譯，而規範明文禁止憑記憶陳述。
- **vault**: 個股評價報告改放 **`stock report\`**（與 `Literature Note\` 同層），檔名統一
  `YYYY-MM-DD 代號`。Literature Note 是知識（寫一次長期有效），個股評價是會過期的觀測
  快照，混在一起會稀釋知識層。`7a Obsidian操作語法` 的 3 系列因此**不新增 `3b`**（已回退）。
- **test**: 新增 4 項（母體標示來源／無條件樣板句已移除／三個資料截止日／涵蓋率警示與
  區間過短回 None）。`pytest tests/ -q` → **414 passed, 2 deselected**。

### v3.38 (2026-08-12)
新增個股評價資料收集器（台股／美股通用），供 `stock-evaluator` agent 使用。
- **feat(scripts)**: `scripts/stock_profile.py` —— 一支代號進、結構化 profile 出
  （`--json` 供程式消費）。**決定性資料收集在程式、判讀留給 agent**：同一支股票同一天
  跑兩次必須得到同一組數字，讓 agent 每次自己拼 SQL 會讓數字隨對話漂移、事後無從追。
  資料源分工：價格走 Yahoo（兩市場同一路徑）／台股技術面走 climber DB **Adj_Close**
  ＋估值籌碼 TDCC／美股僅 OHLCV（**無本地籌碼源，這是事實不是疏漏**）／型態呼叫
  climber `analyzers.panel_indicators` 純函式（兩市場共用同一份判定邏輯）。
- **fix**: 讀 climber DB 時 **open/high/low 與 close 一起乘 `Adj_Close/Close` 還原**。
  只換 close 欄會讓 `|high − close.shift()|` 這類跨欄比較吃到還原因子本身——2330 近 400 根
  的因子自 0.9744 漂到 1.0000，等於憑空生出最多 2.6% 的假跳空與假 true range。
  climber 自身 `build_price_panels` 只換 close，本檔刻意不照抄該混用。
- **新增「成交額分位」，與既有「股數分位」並列**：Yahoo volume 雖已還原股票分割，
  但**股數本身會隨股價長期漂移** → NVDA 分年日量中位數 2016 3.97 億股 → 2026 1.59 億股
  （同期收盤 1.7 → 195.7），股數分位顯示 **0 分位**（十年最低），讀起來像「沒人在交易」，
  實際日成交額 $26.3B 是全市場最活躍之一（**成交額分位 79**）。跨年代比較一律看金額；
  股數分位保留是因為它是 Cow 台股量能維的**校準口徑**（AUC 0.648），不可換單位。
- **刻意不做**：不輸出任何綜合「可炒性總分」。把未回測維度加權成一個分數會讓人以為它
  經過驗證（CONSTITUTION 8-12）。美股也不給雷達分數（2026-07-02 回測 50 檔三維
  AUC~0.5 近雜訊，Cow 已撤下該面板）。
- **test**: 新增 `tests/test_stock_profile.py` 9 項（兆元量級／USD 分級／成交額分位不受
  股數漂移影響〔幾何序列造「金額恆定但股數萎縮」〕／台美流動性門檻不共用／漲跌停欄美股須
  為 None 非 0／週轉率缺股數回 None／ATR 含跳空而振幅不含／融資欄缺失回 None）。
  `pytest tests/ -q` → **410 passed, 2 deselected**。

### v3.37 (2026-08-11)
K 線側欄疊均線（分市場天期）＋ watcher 韌性/限流修正 ＋ 量能分位校準對拍。
- **feat(term-ui)**: `_kline_panel_lines` 新增均線疊圖，天期/字元/顏色由常數單一來源
  （`mas` 參數可覆寫，函式內無硬編碼天期）。各線字元刻意都不同（`·` `-` `+` `=`），
  不是只靠顏色——`_enable_windows_ansi` 對舊主控台是靜默降級，那時顏色全失效。
  新增 `_ma_series`／`_ma_legend_segs`／`_pack_segs`／`_dw_ansi`／`kline_mas_for`。
- **feat(term-ui)**: **分市場均線組**（`kline_mas_for(kind)`）——台股 **5/20/60/240**
  （週/月/季/年線）、美股與幣對 5/20/200。理由見下方「研究」段：台股 243 根/年 →
  年線是 240 不是 200；季線 60 有 94% 的交易日落在近 30 日框內（MA200 只有 39%），
  是真的會跟價格互動的那條。`_print_with_kline` 新增 `mas` 參數，watcher 依 `kind` 傳入。
- **fix(watcher)**: **籌碼源故障不再中斷監控**。`get_chip_bundle` 原本在 `_fetch` 的
  try/except **外面** → TWSE/TPEx 一有連線問題就把例外丟給 `run()`，被當成「擷取失敗」
  走 transient 重試（整頁換成錯誤訊息、等 10 秒、fails+1），但那時價格/趨勢/量能/K 線
  全都是好的。抽出 `_refresh_chip` 吸收例外：**保留上一輪 `_chip`**（TWSE 是 EOD 檔，
  舊一天仍是真資料，畫面 `籌碼資料截至 as_of` 會如實顯示是哪天），說明欄加一行標註，
  下次日線刷新自動再試。以模擬 TWSE 逾時實測確認。
- **fix(watcher)**: **背景標的巡檢節流**。原本每 60s 主迴圈都把 watch_plan 其餘標的全掃
  一遍 → 本標的現價 1 發 + 背景 8 發 = 9 req/min ≈ **540 req/hr**（此 repo 有 Yahoo 429
  前例）。新增 `BG_QUOTE_SEC=300`／`BG_QUOTE_MAX=8`，降到約 2.6 req/min；代價是背景觸價
  最多晚 5 分鐘（本標的仍 60s）。同時修正 `[:9]` 先 slice 後 skip 的差一——改為**先剔本
  標的再取上限**，實際發數恆為 8（原註解宣稱「上限 8 檔」但本標的不在計畫內時會打 9 發）。
  巡檢邏輯抽成 `_background_events` 方法，節流與上限才驗得到（否則測試只能複製一份迴圈
  驗自己寫的副本）。
- **設計（有實證，非拍腦袋）**：
  - **疊圖優先序 K 棒 > 短天期 > 長天期**。同一格只放得下一個字元，K 棒永遠贏——被蓋掉
    就讀不出當天漲跌；均線的關鍵資訊本來就是它沒被 K 棒蓋住的那段（上方＝壓力、下方＝支撐）。
  - **框外均線不畫線，改在圖例標值＋乖離＋`↑`/`↓`**。實測 8 檔 × 10 年逐日，MA 落在近 30 日
    框內的比例：MA5/10/20 **100%**、MA60 94%、MA120 59%、**MA200 僅 39%**、MA240 34%。
    MA200 有六成交易日在框外，硬 clamp 到邊緣會讓「年線遠在下方 24%」看起來像
    「價格剛好站上年線」，是會直接害人誤判的假訊號。
  - **歷史不足以讓整段 n_days 都有值的天期整條略過**（畫半截會被讀成「均線從這天才開始」）。
- **fix(term-ui)**: `_print_with_kline` 的終端寬度判斷由固定 `W + 45` 改為**側欄實寬**
  （`W + 4 + max(_dw_ansi(chart))`）。疊上均線後圖例列可能比格線寬（長天期均線位數多），
  固定門檻會讓側欄溢出換行、整頁對齊全毀。面板寬度同步取「軸標籤+天數」與圖例列的較大者。
- **研究（回答「股市跟 BTC 的 MA 線是否不同」— 實測 Yahoo 10y 日線）**：
  **根/年台股 243、美股 251、幣 365** → **同一條 MA200 在股市涵蓋 9.6~9.9 個月、在幣市只有
  6.5 個月**；MA20 在股市 0.89 個月（台股正是「月線」）、在幣市只有 0.62 個月（不是月線）。
  台股慣例中期線其實是**季線 60 與年線 240**（243 根/年 → 240 才對應一年），200 是美股移植數字。
  → 台股組已改為 5/20/60/240，美股/幣維持 5/20/200。
  **同一個根因的潛在陷阱**：`watcher.py` 的 `win = df.tail(252)` 標成「52週高/低」，對股票
  正確（實測台股 54.0 週、美股 52.4 週）但對幣對只有 **35.9 週**（BTC 現行會顯示 97,861，
  真 52 週高是 126,198，差 29%，而這值直接餵給下面的「位置 %」）。目前不可達
  （`_build_monitor` 把所有幣對都送 `BitcoinMonitor`，`UniversalMonitor` 只收股票），
  **未修**，僅記錄於此——凡是拿固定根數當固定時長，對 24/7 市場一律會錯。
- **verify(calib)**: 重跑 `scripts/tw_volwindow_calib.py`（面板 4,466,012 列／2,079 檔／
  2016-01-04~2026-06-25，test swing high 37,908 含真頂 6,768）→ 逃頂 AUC 1日 **0.648**／
  3日 0.650／5日 **0.648**／10日 0.646／20日 0.638，抄底側全 ≤0.521，**與 v3.35 記載逐格相同**。
  另做**獨立對拍**（不使用產生 v3.36 修改的那套邏輯）：用校準面板資料直接比對 live
  `vol_pctile` vs 校準 `expanding_pctile`，18 檔最大差 **5.83e-04**，且每一檔的差都**恰等於
  1/(2n)**——即 midrank 與 `rank(method="max")` 的解析定義差，代表兩式在無 ties 下完全等價。
  → v3.36 的母體對齊確認落地，量能維門檻無需重訂。
- **test**: 新增 `tests/core/test_kline_ma.py` 11 項（右框線對齊／框內畫線／框外只給圖例＋
  箭頭／逐格對拍 K 棒不被蓋／歷史不足整條略過／`_dw_ansi` 不計色碼／圖例換行／終端過窄退回
  單欄／分市場天期／字元互異／台股四線仍對齊）＋ `test_watcher_stability.py` 6 項（籌碼失敗
  不拋出、保留舊 bundle、成功清旗標、背景上限與剔除本標的、節流窗、事件標記與 state 保留）。
  `pytest tests/ -q` → **401 passed, 2 deselected**。
- **verify**: 離線 render 端到端 —— 2330 台股側欄出現 `·MA5`／`-MA20`／`+MA60` 三條在框內、
  `=MA240 1,818 +31.7%↓` 框外只給圖例；模擬 TWSE 逾時後 `_fetch` 不拋、`_chip` 保留、
  說明欄出現「⚠ 本輪籌碼刷新失敗（TWSE 連線逾時），沿用上表日期資料」。

### v3.36 (2026-08-11)
量能分位母體對齊校準口徑 ＋ 幽靈零量列 ＋ 量比並列顯示。
- **fix(service)**: `fetch_ohlc` 預設 `rng` **2y → 10y**。量能見頂維的分位母體就是這段歷史
  （`vol_pctile` 拿最新值對整段排名），而校準它的 `tw_volwindow_calib` 用 **expanding 分位、
  面板自 2016-01-01 起**（`tw_variant_extract --min-date` 預設）→ live 只餵 2 年等於拿短記憶
  母體去套長記憶母體量出來的門檻（0.95/0.85/0.70）。使用者回報 6782 分位偏高，實查即此：
  同一筆近5日均量 648,800 股，**2 年母體 93.5 分位 vs 10 年母體 83.5 分位**、量能維
  **12/18 vs 6/18**（該股 2025 起量能結構性萎縮，分年日量中位數 2024 520,193 → 2025 213,181，
  2 年窗剛好只裝得下低量期）。價格類指標不受影響（MA/RSI/ATR 固定窗、52週高低取 `tail(252)`）。
  **不可改用 `rng="max"`**：Yahoo 靜默降頻（實測 2330 max 只回 320 根、中位間隔 31 天＝月線；
  6782 315 根＝週線），欄名照舊、量能分位會變成拿週/月量比日量。10y 實測全市場皆日線
  （2330/6509/2454/1101 各 2431 根、AAPL/NVDA 2512、BTC-USD 3653，中位間隔皆 1 天）。
- **fix(service)**: `fetch_ohlc` 把 Yahoo 的**幽靈零量列**（volume=0 但 OHLC 正常、當天實際有
  成交）轉 NaN。實測近 10 年 6782 1 筆（2026-07-10）／2454 4 筆／1101 7 筆／6509 9 筆，
  美股與幣對 0 筆（台股專屬）。轉 NaN 不刪列——價格那根是真的，MA/RSI/ATR 不該少一天；
  量能消費端皆 `mean()`（skipna）或 `dropna()`，NaN 不外溢。
- **fix(core)**: `vol_pctile` 的清洗抽出 `_vol_series` 單一出口，並修正順序為**先剔最後一根、
  再 dropna**（原為先 dropna 再剔）——上一項讓最後一根可能是 NaN，舊順序會在 `drop_last=True`
  時多砍一根已結算日。
- **feat(watcher)**: 量能改**分位＋量比兩行並列**。使用者拿「今日量 ÷ 近5日均量」
  （219,571 ÷ 648,800 = 0.34）去驗算分位並回報「應該是 33 分位」——兩者分母不同、不可互推
  （今日縮量與 5 日均量仍在歷史高檔可同時成立），而 v3.35 已修過一次標籤文字仍再被誤讀 →
  改成兩個問題各給各的數字。新增 `core.relative_high_tw.vol_snapshot`（純顯示不計分，與
  `vol_pctile` 共用 `_vol_series` 故畫面與計分不漂移），分位行改標**實際母體起日與筆數**
  （不再籠統寫「個股自身歷史」）。`ref_window=60` 是判讀慣例非校準值。
- **test**: 新增 4 項（10y 預設與 1d interval 鎖定／幽靈零量列轉 NaN 保留價格列／`_vol_series`
  順序含末根 NaN／量比與分位並存不可互推）。`pytest tests/ -q` → **384 passed, 2 deselected**
  （deselect 為既有 `yahoo` mark 預設不跑）。
- **verify**: 離線 render 實跑 6782 端到端 —— 現價 212.00、`最新日線 …（1455 根）`、
  `量能分位 近5日均量 648,800 股 → 83分位（母體＝2020-08-14 起每日的5日均量 1,432 筆）`、
  `量比 近5日均量 ÷ 近60日均量 2.06x｜今日 219,571 股 ＝ 近5日均量的 0.34x`，
  逃頂面板量能維 `6/18 量能 🟡 偏高 83分位`（改動前為 12/18 93 分位）。
