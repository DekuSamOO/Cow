...
# 數字正本：vault「1a 1 BTC ROAD.md」「二、防守機制」(二)執行順序與強平價推移；驗算：vault「1b 馬丁格爾數學稽核.md」。
# ⚠️ 防守為條件式／決策卡設計說明見 config_private.py.example 與 vault 正本，
#   本檔不再重複列出（數字已不在此處，說明留在私有檔與 vault）。
_DEFENSE_ATTRS = frozenset(
    ("ALERT_PRICE_LOW", "DEFENSE_LADDER", "DEFENSE_DECISION_CARD", "D3_GRID_LIQ_PRICE")
)
_defense_cache = None
...
def _load_defense_config():
    ...
    if json_blob:
        ...
        _defense_cache = (
            ...
            # ⚠️ 2026-09-07：舊的 "mart_tp_baseline" 鍵已隨馬丁哨兵移除，本載入器不再讀取。
            #    Actions secret 若仍留著該鍵，會被直接忽略，不需另外改 secret。
    ...
    _defense_cache = (_alp, _dl, _ddc, _d3glp)
    return _defense_cache


def __getattr__(name):
    if name in _DEFENSE_ATTRS:
        alp, dl, ddc, d3glp = _load_defense_config()
        return {"ALERT_PRICE_LOW": alp, "DEFENSE_LADDER": dl,
                "DEFENSE_DECISION_CARD": ddc,
...
# ── 升槓桿窗口哨兵（2026-08-23 立；正本 vault「1a BTC部位SOP」情境一，證據「1b 要開槓桿時間點策略研究」）──
# 進場閘門＝AHR999 < 門檻 且 距 ATH ≥ 天數。實證（Cow DB 三次熊底 2018/2022/2026）：
# 單條 AHR999<0.40 首訊平均距真底 +31.4%；加距 ATH≥300 天後改善到 +16.6%，
# 且三次全部觸發零錯過。資金費 30 日均<0 更準（+8.8%）但只能驗證 2 次且疊加會錯過本輪，
# 故降級為人工加碼參考，不進哨兵。
LEVERAGE_AHR999_MAX: float = 0.40
LEVERAGE_MIN_DAYS_FROM_ATH: int = 300
# 窗口內分批節奏：回測 6 種方案，每 14 天 1/6（6 批）最佳（平均成本距真底 +12.4%，
...
# ==============================================================================
# "v1"＝舊 _derive_real_season 散裝校正邏輯（保留為 fallback，改回一個字即可）；
# "v2"＝**十六**象限二維狀態機（core/season_forecast.derive_effective_state，設計書
# Github\Cow\season_v2_design.md）——結構性根治 C-2（v1 在「時間入秋、市場未確認」
# 象限會誤出熊底預測）。
#
# ── 2026-09-03：切換為 "v2"（使用者拍板，受保護設定變更）─────────────────────
# 切換前置全部走完，證據 → Github\Cow\歷程\20260902findings_四季論v2象限擴充與回放.md
#   ① 市場軸 3→4 級（補 deep_bear）＋防抖逃生門：既有門檻一個都沒動
#      回放差異 1,439→433 天、observe 型目標格佔比 15.5%→63.5%、發現 B 12→0 天
#   ② 準則 2 修訂為 2a 下跌段須一致／2b 反彈段允許轉 observe：兩輪熊底全過
#      （2018-19 下跌 45 天、2022 FTX 下跌 21 天，**皆 0 分歧**——v2 沒在還在跌時閉嘴）
#   ③ 消費端盤點補了兩個洞：
#      P-1 builders._build_forecast_box 原為二元判斷，observe 會渲染成
#          「🚀 牛市最高價預測 $0/$0/$0」（方向相反且看起來很確定）→ 已補 observe 分支
#      P-2 bottom_floors 不受本開關影響（直接呼叫 project_bear_bottom），
#          observe 時 final_low 仍會印 → 拍板 B 案：保留數字、呈現層加註記
#   驗收：Cow 全套 545 passed, 0 failed
#
# ⚠️ 回滾：改回 "v1" 即可，v1 路徑完整保留、零改動。
SEASON_ENGINE: str = "v2"
