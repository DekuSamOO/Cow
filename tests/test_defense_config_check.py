...
def test_fingerprint_source_agnostic():
    ...
    assert _fingerprint(50000, ladder_t, card_t, mtb) == _fingerprint(50000.0, ladder_l, card_l, mtb)


def test_fingerprint_sensitive_to_each_field():
    """任何一個欄位變動都必須改變指紋（drift 必被抓到）。"""
    base = dict(alp=50000.0, ladder=[[50000.0, 'x', 0.05, 40000.0, 'n']], card=['a'], mtb=None)
    fp0 = _fingerprint(base['alp'], base['ladder'], base['card'], base['mtb'])
    assert _fingerprint(50001.0, base['ladder'], base['card'], base['mtb']) != fp0
    assert _fingerprint(base['alp'], [[50000.0, 'x', 0.06, 40000.0, 'n']], base['card'], base['mtb']) != fp0
    assert _fingerprint(base['alp'], base['ladder'], ['a', 'b'], base['mtb']) != fp0
    assert _fingerprint(base['alp'], base['ladder'], base['card'],
                        {'date': '2026-01-01', 'marts': []}) != fp0
