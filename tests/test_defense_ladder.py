...
# ── 靜態重算警語（2026-09-07：P4 馬丁重啟偵測整組移除後改為固定一行）──────

def test_message_keeps_static_recalc_warning():
    """偵測器沒了，但『執行前必對帳重算』這句提醒不可以跟著消失。"""
    msg = build_defense_message(53_000.0, now_str='TEST')
    assert '馬丁若已止盈重啟' in msg and '執行前必對帳重算' in msg
...
if __name__ == '__main__':
    ...
    test_message_keeps_static_recalc_warning()
    print('防守推移表守門測試通過。')
