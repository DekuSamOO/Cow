...
════════════════════════════════════════════════════════════════════════════
⛔ 2026-08-26 端到端重驗：**本雷達四維全數無訊號，不得作為進場依據**
════════════════════════════════════════════════════════════════════════════
`tests/tw_low_dim_audit.py`（14 檔合併、n=30,955 日、事件 172 次）：

  維度          配分  觸發率   滿分率   AUC     判定
  leverage      40   32.4%   7.7%    0.490   ❌ 比亂猜差（上表宣稱的「最強維 0.564」不成立）
  technical     30   39.1%   0.1%    0.509   ⚠️ 弱
  institution   20   41.3%   17.4%   0.489   ❌ 方向相反
  valuation     10   63.9%   7.4%    0.480   ❌ 方向相反
  總分          100  88.6%   2.7%    0.486

端到端門檻決策品質（`tests/radar_bench_tw.py`）：15/30/45/55/65/75 六個門檻的
**lift 全部落在 0.96~1.15x**（1.0＝與亂猜無異），且 15 分門檻觸發 59% 的交易日＝常亮。
對照同一批資料的台股**逃頂**側：65 分 lift 1.93x —— 同框架、同資料，只有抄底側沒訊號。

**為什麼與上表的 0.564 衝突**：舊校準用**固定 ±18%／60 日**標記真底；
2026-08-26 改用**波動標準化**門檻（k_bot=1.90 x 該標的當下 60 日 σ，
正本 `Github\\Cow\\雷達評估標準.md`）。固定 18% 在台股個股約 0.8~1.3σ，
會把大量「一般回檔」標成真底，樣本被稀釋後任何維度都容易看起來有訊號。
**舊數字不是算錯，是問錯問題。**

處置：**維持顯示、但不得作為判斷依據**（比照 `relative_high_us` 2026-07-04 的處理）。
不重配權重——四維都沒訊號，沒有任何一維可以接收釋出的配額。
重啟條件：找到在波動標準化標記下單維 AUC >= 0.55 的新維度，且 train/holdout 都過。
"""
